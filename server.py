...
def get_country_code(lat, lon):
    ...
    if 35.0 <= lat <= 60.0 and -10.0 <= lon <= 30.0:
        ...
        elif 50.7 <= lat <= 53.6 and 3.3 <= lon <= 7.2: return "NL"
        elif 41.3 <= lat <= 51.1 and -5.1 <= lon <= 9.6: return "FR"
        elif 47.3 <= lat <= 55.1 and 5.8 <= lon <= 15.0: return "DE"
        elif 49.9 <= lat <= 60.9 and -8.6 <= lon <= 1.8: return "GB"
        return "EU"

    # Americas
    if -56.0 <= lat <= 13.0 and -82.0 <= lon <= -34.0:
        if -33.7 <= lat <= 5.3 and -73.9 <= lon <= -34.7: return "BR"
        elif -55.1 <= lat <= -21.8 and -73.6 <= lon <= -53.6: return "AR"
        elif -4.2 <= lat <= 12.5 and -79.0 <= lon <= -66.8: return "CO"
        elif -18.3 <= lat <= 0.0 and -81.3 <= lon <= -68.7: return "PE"
        return "SA"

    if 24.5 <= lat <= 49.0 and -125.0 <= lon <= -66.9: return "US"
...
def analyze_audio_request():
    ...
    native_species_set = get_ebird_species_for_region(country_code)
    is_filtered = native_species_set is not None
    
    print(f"\n📡 [{req_id}] New Request | GPS: ({lat}, {lon}) -> Region: {country_code} | Location Filter Active: {is_filtered} | Start RAM: {current_ram} MB", flush=True)
    ...
    try:
        ...
        results_map = {}
        # 10% minimum probability threshold to discard faint neural network noise
        ...
        for label, score in results_map.items():
            ...
            # eBird Regional Validation:
            # If native eBird species set exists for country, discard non-native species unless score > 85%
            if is_filtered:
                code_lower = species_code.lower()
                is_native = any(
                    code_lower == native_code or code_lower in native_code 
                    for native_code in native_species_set
                )
                if not is_native and raw_prob < 0.85:
                    continue
        ...
        print(f"📊 [{req_id}] Complete in {total_time_ms}ms ({round(total_time_ms/1000, 2)}s) | Evaluated {len(chunks)} windows | Region: {country_code} | Location Filter: {is_filtered} | Peak RAM: {get_ram_usage_mb()} MB", flush=True)
